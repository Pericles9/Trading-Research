...
| **8** | Measurement 1 — concentration | Volume/range concentration curves; burst duration scales; opportunity-decay curve | Concentration curves pooled + per-event; decay curve | Detection latency budget is a number |
| **9** | Measurements 2+3 — spread & impact | Spread by participation bucket; effective spread; impact per signed volume, burst vs. quiet | Spread-vs-participation; impact curves | Compression claim tested; FP cost is a number |
| **10** | Measurement 4 — halts | P(halt \| state); time-to-halt; reopen gap distribution | Reopen gap distribution; halt timing | Sizing constraint is a number |
| **11** | Feature layer | Precomputed event-level feature tables | Feature distributions | Features cached, not recomputed in loops |
| **12** | Event-study grid — T+1 | Markout tables, bucketed, with counts | Markout heatmaps; monotonicity plots | **Does this archive contain an edge at all?** |
| **13+** | Regime labeling → detector → direction → walk-forward | Per §5.3 of the research program | Per phase | Per phase |

Ordering note: **12 comes before the detector work**, deliberately. T+1 is the cleanest surface and the fastest read on whether there's anything here. If the markout grid is flat, you've saved six weeks of detector development.
