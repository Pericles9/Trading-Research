...
- **venv calendar-library drift vs. the phase 1c pin, uncorrected.** `config/phase_1c.json` pins `pandas_market_calendars` 5.4.0 / `exchange_calendars` 4.13.2. The project `.venv` has 5.3.0 / 4.12 installed (confirmed Phase 2 T4). Phase 2 used the installed version rather than upgrading the shared venv, documented the drift in `config/phase_2.json`, and judged the risk low for the 2019-12-01..2026-01-15 derivation range — but the venv itself was never corrected and the drift persists for future phases. — logged Phase 2 T8. **Update, Phase 3 post-approval review:** the risk stopped being hypothetical once T3's classification made expected-session arithmetic load-bearing — checked directly (isolated install of the pinned versions, XNYS session list diffed over the full derivation range): byte-for-byte identical, 1,539/1,539 sessions, 0 diffs. Provably harmless for every expected-session computation used through Phase 3. The venv itself is still uncorrected; this item stays open until it is, but is no longer an unverified risk. See `results/phase_3/artifacts/calendar_pin_verification.json`.
