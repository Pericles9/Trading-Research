...
- **Build F1's financials group cannot be assembled from Massive's endpoint — escalation row 1 fired, Cooper amendment needed.** F1-T0 (the restatement gate test) queried `GET /vX/reference/financials` unfiltered by `filing_date` for two independent universe companies with a confirmed SEC restatement event: AGAE (10-K/A) and CLRB (a genuine 8-K Item 4.02 non-reliance filing). Zero duplicate `(start_date, end_date, timeframe)` records in either company's complete history (18 and 59 records respectively), and CLRB's FY2023 record queried with `filing_date.lt` set before its restatement 8-K returned byte-identical `filing_date`/`revenues` to an unfiltered query — there is no earlier vintage for the filter to reach. **The vendor is not point-in-time**; `fin_` group assembly needs the SEC route (raw XBRL per accepted filing) as its source of record, not Massive's aggregated endpoint, which the original work order left as an open branch pending exactly this test. **F1-T1 through F1-T6 do not run until Cooper amends `prompts/fundamentals_f1.md`'s F1-T5/§4 `fin_` group design.** Also noted in the same investigation, unscheduled: the universe's unique-ticker count is 2,930 by direct count against `results/phase_5/artifacts/quotes_bitmaps_all.parquet`, not the 2,576 the work order's DF-4/D30 text states — the discrepancy is unresolved and does not block F1-T0 itself. Source: `results/fundamentals_f1/artifacts/t0_restatement_test_summary.json`, `research/fundamentals_f1/t0_restatement_test.py`. — logged Build F1 T0, 2026-09-11.
