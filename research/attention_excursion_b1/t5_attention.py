...
def main() -> int:
    ...
    summary = {
        ...
        "a2": {"valid_rungs_distribution": {str(k): int(v) for k, v in dv3["a2_valid_rungs"].value_counts().sort_index().items()},
               ...
               "first_valid_k_distribution": {str(k): int(v) for k, v in dv3["a2_first_valid_k"].value_counts(dropna=False).sort_index().items()},
