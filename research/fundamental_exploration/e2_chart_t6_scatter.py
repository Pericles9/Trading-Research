...
def main():
    df = pd.read_parquet(f"{C.ART_E2}/e2_t6_scatter_events.parquet")
    cap_base = (
        f"sample: D1, n={len(df):,} (per-panel n shown; coverage gaps drop points, never impute). "
        "censored_share is 0 across the population (E2-T2/T3)."
    )
    ...
    SC.continuous_scatter_by_year(
        ...
        x_title="shares outstanding (corrected)", y_title="duration_min (linear -- 52% exactly 0)",
        ...
        out_root=C.CHARTS_E2, out_subdir="e2_t6", out_name="07_duration_vs_shares_outstanding_scatter",
    )
    SC.continuous_scatter_by_year(
        ...
        x_title="filing lag, days + 1", y_title="duration_min (linear -- 52% exactly 0)",
        log_x=True, log_y=False, colorbar_title="price decile",
        ...
        out_root=C.CHARTS_E2, out_subdir="e2_t6", out_name="08_duration_vs_filing_lag_scatter",
    )
    SC.categorical_strip_by_year(
        ...
        y_title="duration_min (linear -- 52% exactly 0)", log_y=False, colorbar_title="price decile",
        ...
        out_root=C.CHARTS_E2, out_subdir="e2_t6", out_name="09_duration_by_dilution_flag_strip",
    )
    SC.categorical_strip_by_year(
        ...
        out_root=C.CHARTS_E2, out_subdir="e2_t6", out_name="10_duration_by_reverse_split_strip",
    )
    SC.categorical_strip_by_year(
        ...
        out_root=C.CHARTS_E2, out_subdir="e2_t6", out_name="11_duration_by_si_quality_strip",
    )
