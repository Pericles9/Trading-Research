...
adjacent-pair check (#e87ba4 vs #e34948, dE 13.2 < 15). No Phase 11 chart uses
...
Palette and helpers carried unchanged from the approved Phase 9 charts
(reuse-before-build); only the config path, chart directory and phase label
differ. The CAT5 cap is retained.
"""
...
CFG = "config/phase_11.json"
CHARTS = "results/phase_11/charts"
...
def caption(sample: str, filters: str, extra: str = "") -> str:
    ...
    if extra:
        lines.append(extra)
    lines.append(f"config {cfg_hash()} · Phase 11")
