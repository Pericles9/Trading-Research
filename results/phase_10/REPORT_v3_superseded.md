...
> [!warning] SUPERSEDED BY D9 — v3 evidentiary record
> Phase 10 **v3 (sub-burst decomposition against the event's own envelope)** was **rejected by
> Cooper on failure row 0**, visual review of chart 07 against the tape. `docs/Universe-Decisions.md`
> **D9** supersedes its method with interval-based detection.
>
> **These v3 results survive D9 and are cited by it:** the Allan/Fano scale-separation gate PASSED
> (a characteristic clustering scale exists — 128 s rth, 16 s premarket, ΔBIC 45.6–68.7), and the
> Arm A test PASSED for the first time in the phase (Spearman 0.2772 / 0.3531). D9's drafted
> consequence (a) said v3 was withdrawn before execution and that no v3 artifacts exist; that is
> corrected in the appended D9 entry, because both results above are load-bearing for D9's own
> argument.
>
> Live spec: `prompts/phase_10_v4.md`. Live report: `results/phase_10/REPORT.md`.


# Phase 10 v3 (SUPERSEDED BY D9, REJECTED ON ROW 0) — Sub-Burst Decomposition
